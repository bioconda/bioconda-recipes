...
(cd kent/src/hg/genePredSingleCover && make CC="${CC}" CXX="${CXX}" CFLAGS="${CFLAGS}" CXXFLAGS="${CXXFLAGS}" -j "${CPU_COUNT}")
cp bin/genePredSingleCover "${PREFIX}/bin"
chmod 0755 "${PREFIX}/bin/genePredSingleCover"
