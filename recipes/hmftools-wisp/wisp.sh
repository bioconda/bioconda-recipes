#!/bin/bash
# hmftools WISP executable shell script
# https://github.com/hartwigmedical/hmftools/tree/master/WISP
set -eu -o pipefail
...
then
    eval "$java" $jvm_mem_opts $jvm_prop_opts -cp "$JAR_DIR/wisp.jar" $pass_args
else
    eval "$java" $jvm_mem_opts $jvm_prop_opts -jar "$JAR_DIR/wisp.jar" $pass_args
fi
exit
