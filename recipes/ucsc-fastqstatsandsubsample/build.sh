...
(cd kent/src/utils/fastqStatsAndSubsample && make CC="${CC}" CXX="${CXX}" CFLAGS="${CFLAGS}" CXXFLAGS="${CXXFLAGS}" -j "${CPU_COUNT}")
cp bin/fastqStatsAndSubsample "${PREFIX}/bin"
chmod 0755 "${PREFIX}/bin/fastqStatsAndSubsample"
