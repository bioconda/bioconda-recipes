...
if [[ "$(uname -m)" == "arm64" ]]; then
    rsync -aP rsync://hgdownload.cse.ucsc.edu/genome/admin/exe/macOSX.arm64/genePredHisto .
    install -v -m 755 genePredHisto "${PREFIX}/bin"
else
    (cd kent/src && make libs PTHREADLIB=1 CC="${CC}" CXX="${CXX}" -j"${CPU_COUNT}")
    (cd kent/src/hg/genePredHisto && make CC="${CC}" -j"${CPU_COUNT}")
    install -v -m 755 bin/genePredHisto "${PREFIX}/bin"
fi
