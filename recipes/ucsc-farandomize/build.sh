...
(cd kent/src/utils/faRandomize && make CC="${CC}" CXX="${CXX}" CFLAGS="${CFLAGS}" CXXFLAGS="${CXXFLAGS}" -j "${CPU_COUNT}")
cp bin/faRandomize "${PREFIX}/bin"
chmod 0755 "${PREFIX}/bin/faRandomize"
