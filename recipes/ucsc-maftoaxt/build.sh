...
if [[ "$(uname -m)" == "arm64" ]]; then
    rsync -aP rsync://hgdownload.cse.ucsc.edu/genome/admin/exe/macOSX.arm64/mafToAxt .
    install -v -m 755 mafToAxt "${PREFIX}/bin"
else
    (cd kent/src && make libs PTHREADLIB=1 CC="${CC}" CXX="${CXX}" -j"${CPU_COUNT}")
    (cd kent/src/hg/mouseStuff/mafToAxt && make CC="${CC}" -j"${CPU_COUNT}")
    install -v -m 755 bin/mafToAxt "${PREFIX}/bin"
fi
