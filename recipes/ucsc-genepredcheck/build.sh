...
(cd kent/src/hg/genePredCheck && make CC="${CC}" CXX="${CXX}" CFLAGS="${CFLAGS}" CXXFLAGS="${CXXFLAGS}" -j "${CPU_COUNT}")
cp bin/genePredCheck "${PREFIX}/bin"
chmod 0755 "${PREFIX}/bin/genePredCheck"
