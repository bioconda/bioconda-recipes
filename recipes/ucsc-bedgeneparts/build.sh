...
if [[ "$(uname -m)" == "arm64" ]]; then
    rsync -aP rsync://hgdownload.cse.ucsc.edu/genome/admin/exe/macOSX.arm64/bedGeneParts .
    install -v -m 755 bedGeneParts "${PREFIX}/bin"
else
    (cd kent/src && make libs PTHREADLIB=1 CC="${CC}" CXX="${CXX}" -j"${CPU_COUNT}")
    (cd kent/src/utils/bedGeneParts && make CC="${CC}" -j"${CPU_COUNT}")
    install -v -m 755 bin/bedGeneParts "${PREFIX}/bin"
fi
