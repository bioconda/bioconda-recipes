...
(cd kent/src/utils/bedGeneParts && make CC="${CC}" CXX="${CXX}" CFLAGS="${CFLAGS}" CXXFLAGS="${CXXFLAGS}" -j "${CPU_COUNT}")
cp -f bin/bedGeneParts "$PREFIX/bin"
chmod 0755 "$PREFIX/bin/bedGeneParts"
