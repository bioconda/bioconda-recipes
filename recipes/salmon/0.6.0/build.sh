...
if [ "$(uname)" == "Darwin" ]; then
    ...
    sed -i 's#find_package(Boost 1.53.0#find_package(Boost 1.74.0#' CMakeLists.txt
    cd build
    cmake -DCMAKE_INSTALL_PREFIX=${PREFIX} -DBOOST_ROOT=$PREFIX -DBoost_NO_SYSTEM_PATHS=ON -DBoost_DEBUG=ON -DBoost_NO_BOOST_CMAKE=ON -DBOOST_LIBRARYDIR=/usr/lib64/boost174 -DBOOST_INCLUDEDIR=/usr/include/boost174 -DBoost_FOUND=TRUE ..
    make install
fi
