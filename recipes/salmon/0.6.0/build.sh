...
if [ "$(uname)" == "Darwin" ]; then
    ...
    cd build
    cmake -DCMAKE_INSTALL_PREFIX=${PREFIX} -DBOOST_ROOT=$PREFIX -DBoost_NO_SYSTEM_PATHS=ON -DBoost_DEBUG=ON -DBoost_NO_BOOST_CMAKE=ON -DBOOST_LIBRARYDIR=/usr/lib64/boost174 -DBOOST_INCLUDEDIR=/usr/include/boost174 ..
    make install
fi
