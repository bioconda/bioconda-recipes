#!/bin/bash
# hmftools protect executable shell script
# https://github.com/hartwigmedical/hmftools/tree/master/protect
set -eu -o pipefail
...
if [[ ${pass_arr[0]:=} == org* ]]
then
    eval "$java" $jvm_mem_opts $jvm_prop_opts -cp "$JAR_DIR/protect.jar" $pass_args
else
    eval "$java" $jvm_mem_opts $jvm_prop_opts -jar "$JAR_DIR/protect.jar" $pass_args
fi
exit
