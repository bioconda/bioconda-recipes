#!/bin/bash
# hmftools Virus Interpreter executable shell script
# https://github.com/hartwigmedical/hmftools/tree/master/virus-interpreter
set -eu -o pipefail
...
if [[ ${pass_arr[0]:=} == org* ]]
then
    eval "$java" $jvm_mem_opts $jvm_prop_opts -cp "$JAR_DIR/virusinterpreter.jar" $pass_args
else
    eval "$java" $jvm_mem_opts $jvm_prop_opts -jar "$JAR_DIR/virusinterpreter.jar" $pass_args
fi
exit
