...
if [[ "$(uname -m)" == "arm64" ]]; then
	rsync -aP rsync://hgdownload.cse.ucsc.edu/genome/admin/exe/macOSX.arm64/bigWigToWig .
	install -v -m 755 bigWigToWig "${PREFIX}/bin"
else
	...
	(cd kent/src/utils/bigWigToWig && make CC="${CC}" CXX="${CXX}" CFLAGS="${CFLAGS}" CXXFLAGS="${CXXFLAGS}" -j"${CPU_COUNT}")
	install -v -m 755 bin/bigWigToWig "${PREFIX}/bin"
fi
