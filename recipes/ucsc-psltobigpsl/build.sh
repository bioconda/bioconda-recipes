...
if [[ "$(uname -m)" == "arm64" ]]; then
	rsync -aP rsync://hgdownload.cse.ucsc.edu/genome/admin/exe/macOSX.arm64/pslToBigPsl .
	install -v -m 0755 pslToBigPsl "${PREFIX}/bin"
else
	(cd kent/src && make libs PTHREADLIB=1 CC="${CC}" CXX="${CXX}" -j"${CPU_COUNT}")
	(cd kent/src/hg/utils/pslToBigPsl && make CC="${CC}" -j"${CPU_COUNT}")
	install -v -m 0755 bin/pslToBigPsl "${PREFIX}/bin"
fi
