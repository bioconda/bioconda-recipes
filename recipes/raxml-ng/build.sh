...
CMAKE_EXTRA_FLAGS="-DCMAKE_POLICY_VERSION_MINIMUM=3.5 -DUSE_VCF=OFF -DUSE_TERRAPHAST=ON -DPORTABLE_BUILD=ON"

# manually disable x86 vector intrinsics on ARM (or any other non-x86 system)
if [ "$(uname -m)" != "x86_64" ]
then
  CMAKE_EXTRA_FLAGS="${CMAKE_EXTRA_FLAGS} -DCORAX_ENABLE_SSE=OFF -DCORAX_ENABLE_AVX=OFF -DCORAX_ENABLE_AVX2=OFF"
fi
