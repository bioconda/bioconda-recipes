...
${PYTHON} -m pip install -vv --no-deps --ignore-installed --no-cache-dir .

mkdir -p ${PREFIX}/bin
mkdir -p ${PREFIX}/bin/share/${PKG_NAME}-${PKG_VERSION}-${PKG_BUILDNUM}/bin

cp "pavfinder/scripts/fusion-bloom" "${PREFIX}/bin/share/${PKG_NAME}-${PKG_VERSION}-${PKG_BUILDNUM}/bin/fusion-bloom"
