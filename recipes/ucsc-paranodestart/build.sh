...
if [[ "$(uname -m)" == "arm64" ]]; then
    rsync -aP rsync://hgdownload.cse.ucsc.edu/genome/admin/exe/macOSX.arm64/paraNodeStart .
    install -v -m 755 paraNodeStart "${PREFIX}/bin"
else
    (cd kent/src && make libs PTHREADLIB=1 CC="${CC}" CXX="${CXX}" -j"${CPU_COUNT}")
    mkdir -p kent/src/parasol/bin
    (cd kent/src/parasol/paraNodeStart && make CC="${CC}" -j"${CPU_COUNT}")
    install -v -m 755 kent/src/parasol/bin/paraNodeStart "${PREFIX}/bin"
fi
