...
(cd kent/src/hg/checkCoverageGaps && make CC="${CC}" CXX="${CXX}" CFLAGS="${CFLAGS}" CXXFLAGS="${CXXFLAGS}" -j "${CPU_COUNT}")
cp bin/checkCoverageGaps "${PREFIX}/bin"
chmod 0755 "${PREFIX}/bin/checkCoverageGaps"
