...
if [[ "$(uname -m)" == "arm64" ]]; then
    rsync -aP rsync://hgdownload.cse.ucsc.edu/genome/admin/exe/macOSX.arm64/overlapSelect .
    install -v -m 755 overlapSelect "${PREFIX}/bin"
else
    (cd kent/src && make libs PTHREADLIB=1 CC="${CC}" CXX="${CXX}" -j"${CPU_COUNT}")
    (cd kent/src/utils && make stringify CC="${CC}" CXX="${CXX}" -j "${CPU_COUNT}")
    (cd kent/src/hg/utils/overlapSelect && make CC="${CC}" -j"${CPU_COUNT}")
    install -v -m 755 bin/overlapSelect "${PREFIX}/bin"
fi
