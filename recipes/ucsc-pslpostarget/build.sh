...
(cd kent/src/utils/pslPosTarget && make CC="${CC}" CXX="${CXX}" CFLAGS="${CFLAGS}" CXXFLAGS="${CXXFLAGS}" -j "${CPU_COUNT}")
cp -f bin/pslPosTarget "$PREFIX/bin"
chmod 0755 "$PREFIX/bin/pslPosTarget"
