...
(cd kent/src/hg/mouseStuff/netToAxt && make CC="${CC}" CXX="${CXX}" CFLAGS="${CFLAGS}" CXXFLAGS="${CXXFLAGS}" -j "${CPU_COUNT}")
cp bin/netToAxt "${PREFIX}/bin"
chmod 0755 "${PREFIX}/bin/netToAxt"
