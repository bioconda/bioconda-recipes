#!/bin/bash
# hmftools Qsee executable shell script
...
then
    eval "$java" $jvm_mem_opts $jvm_prop_opts -cp "$JAR_DIR/qsee.jar" $pass_args
else
    eval "$java" $jvm_mem_opts $jvm_prop_opts -jar "$JAR_DIR/qsee.jar" $pass_args
fi
exit
