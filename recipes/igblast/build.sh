...
# Work directory
RESULT_PATH="$BLAST_SRC_DIR/ReleaseMT"
BUILD_PATH="$RESULT_PATH/build"
BIN_PATH="$RESULT_PATH/bin"
APP_DIR="$BLAST_SRC_DIR/src/app/igblast"
...
LIB_INSTALL_DIR="$PREFIX/lib/ncbi-igblast"
...
# Disable usage reporting to NCBI
export NCBI_USAGE_REPORT_SUPPORTED=0
...
cp $BIN_PATH/makeblastdb $PREFIX/bin/makeigblastdb
cp -R $APP_DIR/internal_data $SHARE_DIR
cp -R $APP_DIR/optional_file $SHARE_DIR
...
# Replace the shebang
sed '1 s_^.*$_#!/usr/bin/env perl_' ${BIN_PATH}/edit_imgt_file.pl > $PREFIX/bin/edit_imgt_file.pl
...
#!/bin/sh
IGDATA="\${IGDATA-"${PREFIX}/share/igblast"}" exec "${SHARE_BIN}/${name}" "\${@}"
EOF
