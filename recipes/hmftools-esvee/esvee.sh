#!/bin/bash
# hmftools Esvee executable shell script
...
pass_arr=($pass_args)
if [[ ${pass_arr[0]:=} == com.hartwig.hmftools.* ]]
then
    eval "$java" $jvm_mem_opts $jvm_prop_opts -cp "$JAR_DIR/esvee.jar" $pass_args
else
    eval "$java" $jvm_mem_opts $jvm_prop_opts -jar "$JAR_DIR/esvee.jar" $pass_args
fi
exit
