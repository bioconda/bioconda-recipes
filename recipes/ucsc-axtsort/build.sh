...
(cd kent/src/hg/mouseStuff/axtSort && make CC="${CC}" CXX="${CXX}" CFLAGS="${CFLAGS}" CXXFLAGS="${CXXFLAGS}" -j "${CPU_COUNT}")
cp -f bin/axtSort "$PREFIX/bin"
chmod 0755 "$PREFIX/bin/axtSort"
