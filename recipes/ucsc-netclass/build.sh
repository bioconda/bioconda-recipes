...
if [[ "$(uname -m)" == "arm64" ]]; then
    rsync -aP rsync://hgdownload.cse.ucsc.edu/genome/admin/exe/macOSX.arm64/netClass .
    install -v -m 755 netClass "${PREFIX}/bin"
else
    (cd kent/src && make libs PTHREADLIB=1 CC="${CC}" CXX="${CXX}" -j"${CPU_COUNT}")
    (cd kent/src/hg/mouseStuff/netClass && make CC="${CC}" -j"${CPU_COUNT}")
    install -v -m 755 bin/netClass "${PREFIX}/bin"
fi
