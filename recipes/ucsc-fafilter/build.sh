...
if [[ "$(uname -m)" == "arm64" ]]; then
    rsync -aP rsync://hgdownload.cse.ucsc.edu/genome/admin/exe/macOSX.arm64/faFilter .
    install -v -m 755 faFilter "${PREFIX}/bin"
else
    (cd kent/src && make libs PTHREADLIB=1 CC="${CC}" CXX="${CXX}" -j"${CPU_COUNT}")
    (cd kent/src/utils/faFilter && make CC="${CC}" -j"${CPU_COUNT}")
    install -v -m 755 bin/faFilter "${PREFIX}/bin"
fi
