...
if [[ ${pass_arr[0]} == eu* ]]
then
    eval "$java" $jvm_mem_opts $jvm_prop_opts -cp "$JAR_DIR/PeptideShaker-1.1.3.jar" $pass_args
else
    eval "$java" $jvm_mem_opts $jvm_prop_opts -jar "$JAR_DIR/PeptideShaker-1.1.3.jar" $pass_args
fi
exit
