...
(cd kent/src/utils/bedRemoveOverlap && make CC="${CC}" CXX="${CXX}" CFLAGS="${CFLAGS}" CXXFLAGS="${CXXFLAGS}" -j "${CPU_COUNT}")
cp -f bin/bedRemoveOverlap "$PREFIX/bin"
chmod 0755 "$PREFIX/bin/bedRemoveOverlap"
