...
if [[ "$(uname -m)" == "arm64" ]]; then
    rsync -aP rsync://hgdownload.cse.ucsc.edu/genome/admin/exe/macOSX.arm64/clusterMatrixToBarChartBed .
    install -v -m 755 clusterMatrixToBarChartBed "${PREFIX}/bin"
else
    (cd kent/src && make libs PTHREADLIB=1 CC="${CC}" CXX="${CXX}" -j"${CPU_COUNT}")
    (cd kent/src/utils/clusterMatrixToBarChartBed && make CC="${CC}" -j"${CPU_COUNT}")
    install -v -m 755 bin/clusterMatrixToBarChartBed "${PREFIX}/bin"
fi
