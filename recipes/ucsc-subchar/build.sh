...
if [[ "$(uname -m)" == "arm64" ]]; then
    rsync -aP rsync://hgdownload.cse.ucsc.edu/genome/admin/exe/macOSX.arm64/subChar .
    install -v -m 755 subChar "${PREFIX}/bin"
else
    (cd kent/src && make libs PTHREADLIB=1 CC="${CC}" CXX="${CXX}" -j"${CPU_COUNT}")
    (cd kent/src/utils/subChar && make CC="${CC}" -j"${CPU_COUNT}")
    install -v -m 755 bin/subChar "${PREFIX}/bin"
fi
