...
(cd kent/src/hg/utils/oligoMatch && make CC="${CC}" CXX="${CXX}" CFLAGS="${CFLAGS}" CXXFLAGS="${CXXFLAGS}" -j "${CPU_COUNT}")
cp -f bin/oligoMatch "$PREFIX/bin"
chmod 0755 "$PREFIX/bin/oligoMatch"
