...
if [[ "$(uname -m)" == "arm64" ]]; then
    rsync -aP rsync://hgdownload.cse.ucsc.edu/genome/admin/exe/macOSX.arm64/hgLoadMaf .
    install -v -m 755 hgLoadMaf "${PREFIX}/bin"
else
    (cd kent/src && make libs PTHREADLIB=1 CC="${CC}" CXX="${CXX}" -j"${CPU_COUNT}")
    (cd kent/src/hg/makeDb/hgLoadMaf && make CC="${CC}" -j"${CPU_COUNT}")
    install -v -m 755 bin/hgLoadMaf "${PREFIX}/bin"
fi
