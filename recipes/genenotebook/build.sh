...
cat scripts/generate-dev-bundle.sh | sed 95,102d | sed 89d | sed 85d | sed 51,52d | sed 50s/.*/mkdir\ -p\ bin\ lib\\/node_modules/ > scripts/gen_dev_bundle.sh && PS4='+(${BASH_SOURCE}:${LINENO}): ${FUNCNAME[0]:+${FUNCNAME[0]}(): }' sh scripts/gen_dev_bundle.sh
