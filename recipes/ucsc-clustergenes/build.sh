...
if [[ "$(uname -m)" == "arm64" ]]; then
    rsync -aP rsync://hgdownload.cse.ucsc.edu/genome/admin/exe/macOSX.arm64/clusterGenes .
    install -v -m 755 clusterGenes "${PREFIX}/bin"
else
    (cd kent/src && make libs PTHREADLIB=1 CC="${CC}" CXX="${CXX}" -j"${CPU_COUNT}")
    (cd kent/src/hg/geneBounds/clusterGenes && make CC="${CC}" -j"${CPU_COUNT}")
    install -v -m 755 bin/clusterGenes "${PREFIX}/bin"
fi
