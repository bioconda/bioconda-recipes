...
mkdir -p $outdir
mkdir -p $PREFIX/bin
cp "$RECIPE_DIR/strling.sh" "$outdir/strling.sh"
ln -s "$outdir/strling.sh" "$PREFIX/bin/strling"
...
install() {
  ...
  if [ "$platform" != "windows_amd64" ]; then
    ...
    export PATH=$nimbleBinDir:"${PREFIX}/bin":$PATH
    say $PATH
  fi

  nimble install -y
  mv "$nimbleBinDir/strling" "$outdir/strling"
}
