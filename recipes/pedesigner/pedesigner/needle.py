...
def needle(a, b, gapopen, gapextend, endgapopen, endgapextend):
    ...
    for i in range(lena):
        p.append(base_to_idx[a[i]])
    for i in range(lenb):
        q.append(base_to_idx[b[i]])
    ...
    cursor = lena * lenb - 1
    if m[cursor] > ix[cursor] and m[cursor] > iy[cursor]:
        score = m[cursor]
    elif ix[cursor] > iy[cursor]:
        score = ix[cursor]
    else:
        score = iy[cursor]
    ...
    while xpos >= 0 and ypos >= 0:
        ...
        if ypos == 0 or ypos == lena - 1:
            FPEQ_a1 = endgapextend
        else:
            FPEQ_a1 = gapextend

        if xpos == 0 or xpos == lenb - 1:
            FPEQ_a2 = endgapextend
        else:
            FPEQ_a2 = gapextend
