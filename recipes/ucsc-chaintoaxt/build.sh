...
if [[ "$(uname -m)" == "arm64" ]]; then
    rsync -aP rsync://hgdownload.cse.ucsc.edu/genome/admin/exe/macOSX.arm64/chainToAxt .
    install -v -m 755 chainToAxt "${PREFIX}/bin"
else
    (cd kent/src && make libs PTHREADLIB=1 CC="${CC}" CXX="${CXX}" -j"${CPU_COUNT}")
    (cd kent/src/hg/mouseStuff/chainToAxt && make CC="${CC}" -j"${CPU_COUNT}")
    install -v -m 755 bin/chainToAxt "${PREFIX}/bin"
fi
