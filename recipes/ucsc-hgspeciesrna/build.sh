...
export LDFLAGS="${LDFLAGS} -L${PREFIX}/lib -headerpad_max_install_names"
...
if [[ "$(uname -m)" == "arm64" ]]; then
    rsync -aP rsync://hgdownload.cse.ucsc.edu/genome/admin/exe/macOSX.arm64/hgSpeciesRna .
    install -v -m 755 hgSpeciesRna "${PREFIX}/bin"
else
    (cd kent/src && make libs PTHREADLIB=1 CC="${CC}" CXX="${CXX}" -j"${CPU_COUNT}")
    (cd kent/src/hg/hgSpeciesRna && make CC="${CC}" -j"${CPU_COUNT}")
    install -v -m 755 bin/hgSpeciesRna "${PREFIX}/bin"
fi
