...
if [[ "$(uname -m)" == "arm64" ]]; then
    rsync -aP rsync://hgdownload.cse.ucsc.edu/genome/admin/exe/macOSX.arm64/axtChain .
    install -v -m 755 axtChain "${PREFIX}/bin"
else
    (cd kent/src && make libs PTHREADLIB=1 CC="${CC}" CXX="${CXX}" -j"${CPU_COUNT}")
    (cd kent/src/hg/mouseStuff/axtChain && make CC="${CC}" -j"${CPU_COUNT}")
    install -v -m 755 bin/axtChain "${PREFIX}/bin"
fi
