#!/bin/bash
# hmftools CIDER executable shell script
# https://github.com/hartwigmedical/hmftools/tree/master/cider
set -eu -o pipefail
...
then
    eval "$java" $jvm_mem_opts $jvm_prop_opts -cp "$JAR_DIR/cider.jar" $pass_args
else
    eval "$java" $jvm_mem_opts $jvm_prop_opts -jar "$JAR_DIR/cider.jar" $pass_args
fi
exit
