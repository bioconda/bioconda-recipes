...
(cd kent/src/hg/mouseStuff/bedCoverage && make CC="${CC}" CXX="${CXX}" CFLAGS="${CFLAGS}" CXXFLAGS="${CXXFLAGS}" -j "${CPU_COUNT}")
cp -f bin/bedCoverage "$PREFIX/bin"
chmod 0755 "$PREFIX/bin/bedCoverage"
