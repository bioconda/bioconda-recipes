...
if [[ "$(uname -m)" == "arm64" ]]; then
	rsync -aP rsync://hgdownload.cse.ucsc.edu/genome/admin/exe/macOSX.arm64/twoBitToFa .
	install -v -m 755 twoBitToFa "${PREFIX}/bin"
else
	...
	(cd kent/src/utils/twoBitToFa && make CC="${CC}" CXX="${CXX}" CFLAGS="${CFLAGS}" CXXFLAGS="${CXXFLAGS}" -j"${CPU_COUNT}")
	install -v -m 755 bin/twoBitToFa "${PREFIX}/bin"
fi
