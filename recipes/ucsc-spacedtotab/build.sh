...
if [[ "$(uname -m)" == "arm64" ]]; then
    rsync -aP rsync://hgdownload.cse.ucsc.edu/genome/admin/exe/macOSX.arm64/spacedToTab .
    install -v -m 755 spacedToTab "${PREFIX}/bin"
else
    (cd kent/src && make libs PTHREADLIB=1 CC="${CC}" CXX="${CXX}" -j"${CPU_COUNT}")
    (cd kent/src/utils/spacedToTab && make CC="${CC}" -j"${CPU_COUNT}")
    install -v -m 755 bin/spacedToTab "${PREFIX}/bin"
fi
