#!/bin/bash
# hmftools TEAL executable shell script
# https://github.com/hartwigmedical/hmftools/tree/master/teal
set -eu -o pipefail
...
then
    eval "$java" $jvm_mem_opts $jvm_prop_opts -cp "$JAR_DIR/teal.jar" $pass_args
else
    eval "$java" $jvm_mem_opts $jvm_prop_opts -jar "$JAR_DIR/teal.jar" $pass_args
fi
exit
