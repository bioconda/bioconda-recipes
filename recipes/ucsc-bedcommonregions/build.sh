...
(cd kent/src/utils/bedCommonRegions && make CC="${CC}" CXX="${CXX}" CFLAGS="${CFLAGS}" CXXFLAGS="${CXXFLAGS}" -j "${CPU_COUNT}")
cp -f bin/bedCommonRegions "$PREFIX/bin"
chmod 0755 "$PREFIX/bin/bedCommonRegions"
