...
(cd kent/src/utils/bedGraphPack && make CC="${CC}" CXX="${CXX}" CFLAGS="${CFLAGS}" CXXFLAGS="${CXXFLAGS}" -j "${CPU_COUNT}")
cp -f bin/bedGraphPack "$PREFIX/bin"
chmod 0755 "$PREFIX/bin/bedGraphPack"
