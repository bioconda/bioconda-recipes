...
(cd kent/src/hg/utils/bedExtendRanges && make CC="${CC}" CXX="${CXX}" CFLAGS="${CFLAGS}" CXXFLAGS="${CXXFLAGS}" -j "${CPU_COUNT}")
cp -f bin/bedExtendRanges "$PREFIX/bin"
chmod 0755 "$PREFIX/bin/bedExtendRanges"
