...
(cd kent/src/hg/mouseStuff/chainFilter && make CC="${CC}" CXX="${CXX}" CFLAGS="${CFLAGS}" CXXFLAGS="${CXXFLAGS}" -j "${CPU_COUNT}")
cp bin/chainFilter "${PREFIX}/bin"
chmod 0755 "${PREFIX}/bin/chainFilter"
