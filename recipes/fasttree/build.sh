...
# build FastTree
${CC} "${LDFLAGS}" -O3 -funsafe-math-optimizations "${EXTRA_ARGS}" $SRC_DIR/FastTree.c -lm -o $PREFIX/bin/FastTree
...
# Build FastTreeMP
${CC} "${LDFLAGS}" -DOPENMP -O3 -fopenmp -funsafe-math-optimizations "${EXTRA_ARGS}" $SRC_DIR/FastTree.c -lm -o $PREFIX/bin/FastTreeMP
