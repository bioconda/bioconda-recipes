...
(cd kent/src/hg/checkHgFindSpec && make CC="${CC}" CXX="${CXX}" CFLAGS="${CFLAGS}" CXXFLAGS="${CXXFLAGS}" -j "${CPU_COUNT}")
cp bin/checkHgFindSpec "${PREFIX}/bin"
chmod 0755 "${PREFIX}/bin/checkHgFindSpec"
