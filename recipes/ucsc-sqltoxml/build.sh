...
if [[ "$(uname -m)" == "arm64" ]]; then
    rsync -aP rsync://hgdownload.cse.ucsc.edu/genome/admin/exe/macOSX.arm64/sqlToXml .
    install -v -m 755 sqlToXml "${PREFIX}/bin"
else
    (cd kent/src && make libs PTHREADLIB=1 CC="${CC}" CXX="${CXX}" -j"${CPU_COUNT}")
    (cd kent/src/hg/sqlToXml && make CC="${CC}" -j"${CPU_COUNT}")
    install -v -m 755 bin/sqlToXml "${PREFIX}/bin"
fi
