...
echo -e '>1\nAAA' > test.fa
mccortex 31 build -k 5 --sample test_sample -1 test.fa test.ctx

# but I suppose we can get around that by greping for usage
# let's make sure to check both k-mer sizes were compiled
set +o pipefail
mccortex 31 2>&1 | grep -F 'usage: mccortex31 <command> [options] <args>'
mccortex 63 2>&1 | grep -F 'usage: mccortex63 <command> [options] <args>'
set -o pipefail
